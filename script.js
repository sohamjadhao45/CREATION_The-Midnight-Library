/* =====================================================================
   ...
   Cache-Nuke | Canvas Renderer | AI Narrator | Interactive Seals
   ===================================================================== */

// 🔴 THE CACHE KILLER: Yeh code har user ke phone se purana PWA nikaal fekega!
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* ======================================================
       📜 POEM DATABASE (YOUR ORIGINAL DATABASE)
       ====================================================== */
    const POEM_DATABASE = [
        {
        ...
        if(enterBtn) {
            enterBtn.addEventListener("click", () => {
                ...
                if(audioAmbient && !globalState.isAudioPlaying) {
                    ...
                    audioAmbient.play().catch(e => console.log("Audio blocked"));
    ...
    // 🔴 🔴 THE MASTER CANVAS GENERATOR (Immune to all CSS Squish Bugs) 🔴 🔴
    ...
    document.body.addEventListener('click', async function(e) {
        ...
        if(e.target && e.target.classList.contains('download-poem-btn')) {
            ...
            link.download = `${poem.title.replace('<br>','')}-Memory.png`; 
            link.href = canvas.toDataURL("image/png"); 
            link.click();
            showToast("📸 High-Quality Memory Saved Successfully!");
            return;
        }

        // 🔗 2. PERFECT SHARE BUTTON (Sends actual image blob via WhatsApp/Native Share)
        if(e.target && e.target.classList.contains('share-poem-btn')) {
            ...
            canvas.toBlob((blob) => {
                ...
                if (navigator.canShare && navigator.canShare({ files: [file] })) {
                    navigator.share({
                        files: [file],
                        title: poem.title.replace('<br>', ' '),
                        text: `A whisper from Soham's sanctuary... 🌙`
                    }).then(() => showToast("✨ Card shared successfully!")).catch(err => console.log(err));
                } else {
                    // Fallback for browsers that block direct image sharing
                    navigator.share({
                        title: poem.title.replace('<br>', ' '),
                        text: `📖 *${poem.title.replace('<br>', ' ')}*\n\n"${poem.text}"\n\nRead more at The Midnight Library: ${window.location.href}`
                    }).catch(err => {
        ...
        if(e.target && e.target.classList.contains('listen-btn')) {
            ...
            if(!page) return;
            const pIdx = page.getAttribute('data-poem-index');
            const poem = POEM_DATABASE[pIdx];
            if(!poem) return;

            let utterance = new SpeechSynthesisUtterance(poem.text);
            utterance.rate = 0.85; 
            utterance.pitch = 0.9; 
