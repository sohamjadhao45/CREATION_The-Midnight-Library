/* =====================================================================
   THE MIDNIGHT LIBRARY ENGINE (THE TRUE MASTERPIECE EDITION)
   JSON Fetching | HQ Canvas Render | Author's Desk | PWA Cache Nuke
   ===================================================================== */

// 🔴 THE CACHE KILLER: Purane PWA ko system se nikaal fekega
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.getRegistrations().then(function(registrations) {
        for(let registration of registrations) {
            registration.unregister();
            console.log('🗑️ Rogue Service Worker Terminated.');
        }
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Initialize the core visuals immediately
    ...
    initTouchRipple(); 
    init1111Wish();
    initAuthorsDesk(); // Start the hidden admin panel logic
    
    // Fetch JSON and build the poems
    loadLibraryData();

    /* --- SAFE INSTALL LOGIC (Fallback) --- */
    let deferredPrompt;
    ...
    /* --- CORE ENGINES & SECRETS --- */
    function initTouchRipple() {
        document.body.addEventListener('click', (e) => {
            if(e.target.tagName === 'BUTTON' || e.target.closest('.btn-icon')) return;
    ...
    function buildLibrarySystem() {
        ...
        if(starMap) {
            for (let i = 0; i < POEM_DATABASE.length; i++) {
                let p1 = starCoords[i % starCoords.length]; starsHtml += `<div class="star-node active-star trigger-nav" data-target="poem-page-${i+1}" title="${POEM_DATABASE[i].dateText} - ${POEM_DATABASE[i].title.replace('<br>',' ')}" style="top: ${p1.top}%; left: ${p1.left}%;"></div>`;
    ...
    if(thoughtBtn && thoughtDisplay) {
        thoughtBtn.addEventListener("click", () => {
            thoughtDisplay.style.opacity = 0; setTimeout(() => { thoughtDisplay.innerText = `"${midnightThoughts[Math.floor(Math.random() * midnightThoughts.length)]}"`; thoughtDisplay.style.opacity = 0.8; }, 300);
        });
    }
    ...
    function toggleRain() {
        ...
        if(globalState.rainActive) { 
            rCanvas?.classList.add("raining"); startRainVisuals(); showToast("🌧️ The sky begins to weep..."); 
            if(audioRain) { audioRain.volume = 0.5; audioRain.play(); }
        } else { 
            rCanvas?.classList.remove("raining"); showToast("🌤️ The storm has passed."); 
    ...
    function init1111Wish() {
        ...
        setInterval(() => {
            const d = new Date();
            if (d.getHours() === 23 && d.getMinutes() === 11 && !globalState.elevenElevenTriggered) { globalState.elevenElevenTriggered = true; if(modal) modal.style.display = "flex"; }
            if (d.getMinutes() !== 11) globalState.elevenElevenTriggered = false; 
        }, 10000);
        if(submitBtn) { submitBtn.addEventListener("click", () => { if(input.value.trim() !== "") { modal.style.display = "none"; showToast("Keep visiting, your wish will be completed soon wanderer. ✨"); } }); }
    }
    ...
    function initUltimateUniverseBackground() {
        ...
        const hour = new Date().getHours(); const isLateNight = hour >= 23 || hour <= 4; const darkThemeBgColor = isLateNight ? "#030308" : "#050507"; 
    ...
    function bindWaxSeals(page) {
        ...
        sealBtn.addEventListener('click', (e) => {
            sealWrap.dataset.isBroken = "true"; sealWrap.classList.add("broken"); showToast("🔓 The seal is broken."); setTimeout(initTypewriterEngine, 800);
        });
    }
    ...
    function applyWhispers(el, poemIndex) {
        ...
        el.querySelectorAll('.whisper-word').forEach(span => {
            span.addEventListener('click', function() {
                let curr = this.innerText; this.style.opacity = 0; setTimeout(() => { this.innerText = (curr.toLowerCase() === this.dataset.original.toLowerCase()) ? this.dataset.hidden : this.dataset.original; this.style.opacity = 1; this.classList.toggle('whispered'); }, 300);
            });
        });
    }
    ...
    /* --- THE FLAWLESS OFF-SCREEN HQ CAPTURE ENGINE (PURE CANVAS DRAWING) --- */
    function createPoemCanvas(poem) {
        ...
        ctx.fillText(poem.title.replace('<br>', ' '), canvas.width / 2, 400);
    ...
    /* --- GLOBAL BUTTON LISTENERS (Save, Share, Audio, Favs, Bookmarks) --- */
    document.body.addEventListener('click', (e) => {
        // DOWNLOAD
        if(e.target.classList.contains('download-poem-btn')) {
            ...
            const link = document.createElement("a"); link.download = `Midnight_Memory_${Date.now()}.png`; link.href = canvas.toDataURL("image/png"); link.click(); showToast("✨ Memory Saved to Gallery!");
        }
        // SHARE
        if(e.target.classList.contains('share-poem-btn')) {
            ...
            canvas.toBlob((blob) => {
                ...
                if (navigator.canShare && navigator.canShare({ files: [file] })) {
                    navigator.share({ files: [file], title: poem.title.replace('<br>', ' '), text: `A whisper from Soham's sanctuary... 🌙` }).then(() => showToast("✨ Shared successfully!")).catch(() => {});
                } else {
                    const fallbackText = `📖 *${poem.title.replace('<br>', ' ')}*\n\nRead this beautiful verse at The Midnight Library: ${window.location.href}`;
                    navigator.share({ title: poem.title.replace('<br>', ' '), text: fallbackText }).catch(() => { navigator.clipboard.writeText(fallbackText); showToast("🔗 Link copied to clipboard!"); });
                }
            }, 'image/png');
        }
        // AUDIO
    ...
    function updateSavedDrawers() {
        ...
        document.querySelectorAll('.bookmark-item[data-title]').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                const poemIndex = POEM_DATABASE.findIndex(p => p.spineLabel === targetTitle || p.title.replace('<br>', ' ') === targetTitle);
                if(poemIndex !== -1) {
        ...
        if(moonTrigger) {
            moonTrigger.addEventListener("click", () => {
                ...
                if(globalState.secretClicks === 3) { 
                    globalState.hasTappedMoon = true; checkUltimateVault(); showToast("🏆 Achievement Unlocked: Moonwalker");
    ...
    /* ======================================================
       🖋️ THE AUTHOR'S DESK (HIDDEN ADMIN PANEL)
       ====================================================== */
    function initAuthorsDesk() {
        ...
        genBtn.addEventListener('click', () => {
            ...
            if(!ch || !ttl || !txt) {
                showToast("⚠️ Missing vital parameters.");
                return;
            }
